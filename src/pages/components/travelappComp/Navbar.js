...
const Sidebar = () => {
  ...
  {showSidebar ? (
    <button
      className="flex text-4xl text-white items-center cursor-pointer fixed right-2 top-4 z-50 text-[40px]"
    ...
    <button
      onClick={() => setShowSidebar(!showSidebar)}
      className="fixed z-30 flex items-center cursor-pointer right-2 top-4"
      viewBox="0 0 100 80"
      width="40"
      height="40"
    >
    ...
    <div className={`top-0 right-0 w-full bg-gradient-to-l from-neutral-900 to-transparent p-10 pl-20 text-white fixed h-full z-40 ${showSidebar ? "translate-x-0 " : "translate-x-full"} ease-in-out duration-300`}>
                <ul className='list-none flex flex-col justify-end place-items-end mr-6 mt-10 flex-1 ;'>
