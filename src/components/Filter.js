...
function Filter({ setActiveCategory, activeCategory, setFiltered, setfilteredPDF, allProjects, allPDFProjects })
{
  ...
  return (
    ...
    className='text-black mb-20 flex justify-between lg:gap-6 md:gap-4 gap-2 lg:text-[15px] md:text-[13px] text-xs text-center'>
        ...
        <button onClick={() => setActiveCategory(4)} className={`p-2 lg:w-28 w-20 text-black rounded-2xl font-bold ${activeCategory === 4 ? "bg-[rgb(65,98,168)]" : "bg-white"}`}>Full Stack</button>
        <button onClick={() => setActiveCategory(5)} className={`p-2 lg:w-28 w-20 text-black rounded-2xl font-bold ${activeCategory === 5 ? "bg-[rgb(65,98,168)]" : "bg-white" }`}>Frontend</button>
